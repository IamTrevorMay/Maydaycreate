...
interface TrainingWebSocketState {
  connected: boolean;
  send: (message: BridgeMessage) => void;
  onMessage: (type: string, callback: MessageCallback) => () => void;
}

export function useTrainingWebSocket(): TrainingWebSocketState {
          ...
          if (cbs) {
            for (const cb of cbs) {
              try { cb(message.payload); } catch (err) {
                console.error(`[Training] Listener error for ${message.type}:`, err);
              }
            }
          }
        } catch (err) {
